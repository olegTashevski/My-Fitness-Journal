...
function firstSliceOfPostsMainUser() {
  getPostsForMainUser();
  if (lastPost) {
    return;
  }
...
function ifNotTheFirstSliceOfPostsMainUser() {
  if (lastPost) {
...
function getPostsForMainUserAfterScrolledBottom() {
  if (firtTime) {
    firstSliceOfPostsMainUser();
  }

  else {
    ifNotTheFirstSliceOfPostsMainUser();
  }
}


function ifFirstSliceOfPostsForFriends() {
  getPostsForFriends();
  if (lastPost) {
    return;
  }
...
function ifNotFirstSliceOfPostsForFriends() {
...
function getPostsForFriendsAfterScrolledBottom() {
  ...
  if (firtTime) {
    ifFirstSliceOfPostsForFriends();
  }

  else {
    ifNotFirstSliceOfPostsForFriends();
  }
...
async function submitPostEdit(evt) {
  let bodyI = new FormData();
  const postId = evt.currentTarget.postId;
  bodyI.append("postId", postId);
  const newHead = document.getElementById("headEdit").value;
  bodyI.append("newHead", newHead);
  const newDescription = document.getElementById("comment").value;
  bodyI.append("newDescription", newDescription);
  let newImagesI = document.getElementById("imagesForPostEdit").files;
  if (newImagesI.length != 0) {
    for (const image of newImagesI) {
		bodyI.append("newImages[]", image);
    }
  ...
  let response = await fetch("/postApi/editPost", {
    method: "POST",
    body: bodyI
  });
